"""Core LangGraph ticket processing logic"""
...
Categories: technical, billing, account, feature_request, bug_report, other"""
    result = json.loads(_chat(system, state.user_message))
...
Priorities: critical, high, medium, low"""
    result = json.loads(_chat(system, f"Category: {state.category}\nMessage: {state.user_message}"))
...
Reply with JSON only: {"score": <0-1>}"""
    result = json.loads(_chat(system, f"Ticket: {state.user_message}\nResponse: {state.response}"))
