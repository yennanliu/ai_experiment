...
Intelligent model routing for Claude API to optimize costs while maintaining quality.

## Overview

This module implements multiple routing strategies to automatically select the appropriate Claude model (Haiku, Sonnet, or Opus) based on task complexity, achieving up to **91% cost savings** compared to using Opus for all requests.
...
Uses Haiku (~$0.001/request) to classify task complexity and category.
...
# -> ModelTier.OPUS (classified as expert-level)
```

### 3. Dynamic Quality Escalation
Starts with Haiku, escalates if quality is insufficient.
...
# Automatically escalates if Haiku's response quality < 70
```
...
print(f"Savings: {estimates['savings_vs_opus']}%")
```
...
|------------|-------|-----------|
| Trivial | Haiku | Simple Q&A, translations, extraction |
| Simple | Haiku | Formatting, summarization, basic tasks |
| Moderate | Sonnet | Code generation, analysis, multi-step |
| Complex | Sonnet | Debugging, review, detailed analysis |
| Expert | Opus | Security, proofs, architecture design |

## Typical Distribution

Based on analysis, ~70% of tasks don't need the strongest model:

- **Haiku**: 70% (trivial + simple tasks)
- **Sonnet**: 25% (moderate + complex tasks)
- **Opus**: 5% (expert tasks only)
