...
The measured rate selects `stop` on its own, and a single failing answer would
select it again by a different clause. Expand and revise can be argued after
the fact; stop is the decision somebody will want to relitigate, so its
condition -- a traced rate below **0.75** -- is the one that has to exist
before the run.
...
def verify(result):
    return [
        practice.Check(
            "ANSWER: expand, revise and stop, with the real run landing on stop",
            all([result["paths"] == 3, result["total"] == 50, result["passing"] == 50,
                 result["traced"] == 0.708, result["decision"] == "stop"]),
            f"{result['passing']} of {result['total']} shipped answers pass and the traced "
            f"rate is {result['traced']}, below the stop threshold of "
            f"{result['stop_threshold']} and well under the expand threshold of "
            f"{result['expand_threshold']}, so the rule says {result['decision']}",
        ),
        practice.Check(
            "FINDING: the control is a string and the criteria are data",
        ...
        practice.Check(
            ...
            "the rate alone already selects stop, and one failing answer would select it "
            "again by a different clause; expand and revise can be argued afterwards, so "
            "the threshold that has to exist before the run is the stop one",
        ),
        practice.Check(
