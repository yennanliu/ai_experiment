...
substitution costs nine lines. The learned arm is run through the same `rope`
call with the angle set to zero, so the two arms execute one code path and the
only difference between them is the table.
...
def step(np, idx, target, w, vocab, trig, backward=True):
    ...
    n, zero = idx.shape[1], np.zeros((BLOCK, WIDTH))
    x, cache, mask = w["tok"][idx] + w.get("pos", zero)[:n], [], np.tril(np.ones((n, n)))
    for i in range(LAYERS):
        q, k, v = (split(t) for t in np.split(x @ w[f"qkv{i}"], 3, axis=-1))
        q, k = rope(np, q, trig), rope(np, k, trig)
    ...
    for i in reversed(range(LAYERS)):
        ...
        cat = np.concatenate([merge(t, xi.shape) for t in
                              (rope(np, dq, trig, -1.0), rope(np, dk, trig, -1.0), dv)], -1)
...
def train(np, ref, mode, steps, seed=0, report=()):
    ...
    theta = theta if mode == "rope" else np.zeros_like(theta)   # identity: cos 1, sin 0
    ...
    for t in range(1, steps + 1):
        ...
        loss, G = step(np, idx, target, w, vocab, trig)
        ...
        if t in report:                        # 20 batches covers all 36 val windows
            ...
            seen[t] = sum(step(np, *batch(np, val, data, split_at, "val"), w, vocab, trig,
