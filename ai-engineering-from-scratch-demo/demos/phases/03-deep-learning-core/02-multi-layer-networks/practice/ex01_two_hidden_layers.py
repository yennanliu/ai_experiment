...
Reading of the exercise: "see how the representation transforms" is the point, so the
checks measure the transform rather than narrate it — check 2 tracks the distance between
the two inputs XOR has to separate, layer by layer. It turns out to be a contraction, so
checks 3-5 give the control, the mechanism and the consequence; check 6 is the shape
mismatch the lesson's objectives ask the reader to spot.
"""
...
def solve():
    ...
    return {
        ...
        "deep": deep, "shallow": contraction(ref, 1, SHALLOW), "truncated": truncated(ref),
...
def verify(result):
    ...
    layers, (wide, right) = " -> ".join(f"{g:.4f}" for g in deep), result["truncated"]
    return [
        ...
        practice.Check("CONTROL: one hidden layer less and the same pair stays "
                       ...
                       f"2-4-1 under identical init ends {result['shallow'][2]:.4f} apart against "
                       f"{deep[3]:.4f} for 2-4-2-1 — the loss is the extra layer, not the fixture"),
        practice.Check("MECHANISM: every layer shrinks distances, and the origin carries none",
                       max(ratios) < 0.27 and all(h == [0.5] * 4 for h in result["origins"]),
                       f"per-layer ratios {ratios[0]:.3f}, {ratios[1]:.3f}, {ratios[2]:.3f} — "
                       f"sigmoid is 1/4-Lipschitz and U(-1, 1) weights have mean |w| = 0.5, so the "
                       f"composed map is a contraction. Separately the default biases are all 0.0, "
                       f"so [0, 0] leaves the first hidden layer as exactly [0.5]*4 in all 20 seeds"),
        practice.Check("ANSWER: no random 2-4-2-1 net gets XOR right",
                       result["flat"] == 0 and result["sharp"] > 0,
                       f"{result['flat']} of {WIDE} nets at the lesson's U(-1, 1) init solve all "
                       f"four rows; at U(-20, 20), the magnitude the lesson hand-tunes to, "
                       f"{result['sharp']} of {WIDE} do — depth alone buys nothing, the weights "
                       f"have to survive the contraction"),
        practice.Check("FINDING: the forward pass never checks a dimension",
                       wide == right,
                       f"a middle Layer(8, 2) fed only 4 values returns {wide:.16f}, bit-identical "
                       f"to the correctly wired Layer(4, 2), and raises nothing. MECHANISM: "
                       f"`forward` pairs weights to inputs with zip(), which stops at the shorter — "
                       f"the four unmatched weights per neuron are silently dropped"),
    ]
