...
same binary cross-entropy, so no code is forked — check 1 shows the loss equals lesson 05's
`label_smoothed_cce` and that lesson 07's `backward` already differentiates it. "The final
train-test accuracy gap" has two readings: check 2 measures both halves in eval mode, check
4 reads it off the lesson's own printout, and they pick opposite winners.
"""
...
def solve():
    ...
    return {"rows": {name: average(runs) for name, runs in raw.items()},
            ...
            "loss_gap": smoothing_gap(l05)}


def smoothing_gap(l05) -> float:
    """The lesson's own label_smoothed_cce against BCE at the smoothed target."""
    return max(abs(l05.label_smoothed_cce([0.0, z], 1, 2, alpha=ALPHA)
                   - bce(1 / (1 + math.exp(-z)), 1 - ALPHA / 2)) for z in (-2.0, 0.7, 3.0))


def verify(result):
    rows, spread = result["rows"], result["spread"]
    ne, sm, both = rows["neither"], rows["smoothing"], rows["both"]
    best = min(rows, key=lambda n: rows[n]["gap"])
    shown_best = min(rows, key=lambda n: rows[n]["shown_gap"])
    rival = max(rows[n]["test"] for n in rows if n != "neither")
    return [
        practice.Check("CONTROL: two-class smoothing is BCE on a softened target",
                       result["loss_gap"] < 1e-12 and result["grad"] < 1e-8,
                       f"lesson 05's label_smoothed_cce(alpha={ALPHA}) is BCE at target "
                       f"{1 - ALPHA / 2} to {result['loss_gap']:.1e}, and lesson 07's own "
                       f"`backward` differentiates it to {result['grad']:.1e} of a difference"),
        practice.Check("ANSWER: 'neither' gives the smallest gap — every regularizer widens it",
                       best == "neither",
                       f"eval-mode train minus test accuracy — {_table(rows, 'gap')}; the dropout "
                       "rows vary by " + ", ".join(f"{n} ±{s:.2f}" for n, s in spread.items())
                       + " over 6 mask seeds"),
        practice.Check("FINDING: regularization does not close the gap — it costs test accuracy",
                       ne["test"] > rival,
                       f"test accuracy {_table(rows, 'test', '%')}. The plain net is already at "
                       f"{ne['gap']:+.2f} pp on 150 test points — no overfitting to remove, so "
                       f"every constraint added is capacity lost"),
        practice.Check("FINDING: read off the lesson's own printout the answer reverses",
                       shown_best == "both" and both["shown_gap"] < 0,
                       f"train_model grades the training set through the dropout mask, so its "
                       f"printed gap= line reads {_table(rows, 'shown_gap')} and picks "
                       f"{shown_best} at {both['shown_gap']:+.2f} pp — an artefact"),
        practice.Check("MECHANISM: smoothing buys the gap with confidence, and log-loss bills it",
                       sm["test_loss"] > 2 * ne["test_loss"]
                       and 1 - sm["top"] > 1e5 * (1 - ne["top"]),
                       f"top test prediction {ne['top']:.10f} unsmoothed against {sm['top']:.4f} "
                       f"smoothed — a {1 - ALPHA / 2} ceiling it still overshoots; test loss on "
                       f"hard labels {sm['test_loss']:.4f} vs {ne['test_loss']:.4f}"),
    ]
