...
Reading of the exercise: Nesterov changes *where the gradient is sampled*, not the update rule,
so this reuses `SGDMomentum` untouched. "Compare convergence" is answered twice — on the circle
dataset at lr = 0.05 (check 1), and in closed form on the quadratic the lesson's own `__main__`
minimises, where both methods are exact two-term recurrences (2-3). Check 4 keeps check 1 honest.
"""
    ...
    for _epoch in range(epochs):
        ...
        for point, label in data:
            ...
            if lookahead and opt.velocities is not None:
                net.set_params([p - opt.lr * opt.beta * v for p, v in zip(params, opt.velocities)])
            net.forward(point); grads = net.compute_grads(label)      # noqa: E702
...
def analyse(errors, a, b):
    ...
    return (max(abs(errors[t + 1] - a * errors[t] + b * errors[t - 1])
                for t in range(1, len(errors) - 1)), max(abs((a + root) / 2), abs((a - root) / 2)))


def solve():
    ref = parity.load_reference(PHASE, LESSON, "main")
    data = ref.make_circle_data()
    arms = [("momentum", LR, False), ("nesterov", LR, True), ("tuned", 0.01, False)]
    runs = {n: train(ref, data, ref.SGDMomentum(lr=lr, beta=BETA), look) for n, lr, look in arms}
    return {"runs": runs, "rec": {"momentum": (1.7, BETA), "nesterov": (1.52, 0.72)},
            "errors": {k: quadratic(ref, k) for k in ("sgd", "momentum", "nesterov")}}


def digest(runs, errors, rec) -> dict:
    """Every number `verify` compares, computed here so that stays a list of comparisons."""
    fit = {k: analyse(errors[k], *rec[k]) for k in rec}
    return {"end": {k: f"{v[-1][0]:.4f} / {v[-1][1]:.1f}%" for k, v in runs.items()},
            "best": max(a for _l, a in runs["momentum"]), "worst": max(v[0] for v in fit.values()),
            "rate": {k: v[1] for k, v in fit.items()},
            "exact": max(abs(e - 7.0 * 0.8 ** t) for t, e in enumerate(errors["sgd"])),
            "size": ", ".join(f"{abs(errors[k][40]):.2e}" for k in ("sgd", "nesterov", "momentum"))}


def verify(result):
    runs, d = result["runs"], digest(result["runs"], result["errors"], result["rec"])
    end, rate, loss = d["end"], d["rate"], {k: v[-1][0] for k, v in runs.items()}
    return [
        practice.Check("ANSWER: the lookahead converges faster at the lesson's own lr = 0.05",
                       ...
                       f"90% at epoch 3 against 6; after {EPOCHS} epochs {end['nesterov']} against "
                       f"{end['momentum']}. Momentum never reaches 95% (best {d['best']:.1f}%); "
                       f"Nesterov does at epoch {reached(runs['nesterov'], 95)}"),
        practice.Check("MECHANISM: 1-2*lr factors out of both coefficients", d["worst"] < 1e-13,
                       f"on f(x) = (x-3)^2 from x = 10 at lr = 0.1: momentum obeys e[t+1] = 1.70*"
                       f"e[t] - 0.90*e[t-1], Nesterov 0.8*(1.90*e[t] - 0.90*e[t-1]); worst residual "
                       f"over 40 steps {d['worst']:.2e}"),
        practice.Check("FINDING: on that quadratic plain SGD beats both",
                       abs(rate["momentum"] - 0.9487) < 5e-4 and d["exact"] < 1e-13
                       and abs(rate["nesterov"] - 0.8485) < 5e-4,
                       f"per-step rates: SGD 0.8000 (exactly 7*0.8^t, worst deviation "
                       f"{d['exact']:.1e}), Nesterov {rate['nesterov']:.4f}, momentum "
                       f"{rate['momentum']:.4f} = sqrt(beta); |e40| = {d['size']}, momentum 873x "
