...
def solve():
    ...
    return dict(track(ref), steady=probe(ref, [2.5] * 10), decay=tail[100] / tail[99],
                ...
                ceiling=(1 - ad.beta1) / math.sqrt((1 - ad.beta2) * (1 - ad.beta1**2 / ad.beta2)))
...
def verify(result):
    ...
    return [
        practice.Check("ANSWER: no — the fastest parameter outruns the slowest by 20x or more",
                       ...
                       f"|effective rate| / lr as min/median/max over 33 parameters, step "
                       f"{shape}; the median also drops "
                       f"{snaps[10]['mid'] / snaps[200]['mid']:.1f}x from step 10 to 200"),
        practice.Check("FINDING: most parameters have gradient exactly 0 and are updated anyway",
                       ...
                       "24 of 33 gradients are exactly zero at step 50, 18 of those move by"
                       " over 0.1 lr (above): ReLU zeroes a unit with z <= 0, Adam uses m"),
        practice.Check("MECHANISM: the ratio measures agreement, not gradient size",
                       ...
                       f"decays {result['decay']:.4f} per step towards {result['limit']:.4f} = "
                       f"beta1/sqrt(beta2), {result['quiet']} steps to reach 0.01 lr"),
        practice.Check("FINDING: lr is not a ceiling on Adam's step",
                       ...
                       f"{result['peak']:.4f} lr, against a Cauchy-Schwarz bound of "
                       f"(1-beta1)/sqrt((1-beta2)(1-beta1^2/beta2)) = {result['ceiling']:.2f} lr"),
