...
to the exact identity dL/dz = softmax - onehot. Checks 4-6 are about `vmap`
itself: whether it reproduces the loop, and what it actually costs.
"""
...
def fixture(seed, n, noise=4.0):
    ...
    teacher = jax.random.normal(jax.random.PRNGKey(3), (784, 10))
...
def profile(ref, params, xs, ys):
    ...
    hit = jnp.argmax(probs, axis=-1) == ys
    return {"norms": norms, "spread": float(norms.max() / norms.min()),
            "r_loss": pearson(norms, -jnp.log(probs[jnp.arange(BATCH), ys])),
            "r_delta": pearson(norms, jnp.linalg.norm(probs - jax.nn.one_hot(ys, 10), axis=1)),
            "wrong": float(norms[~hit].mean()), "right": float(norms[hit].mean()),
            "n_wrong": int((~hit).sum()),
            "bias_gap": float(jnp.max(jnp.abs(grads["layer3"]["b"]
                                              - (probs - jax.nn.one_hot(ys, 10))))),
...
def race(ref, params, xs, ys):
    ...
    batched = jax.jit(jax.vmap(single, in_axes=(None, 0, 0)))
    per_ex = jax.jit(single)
    vec, loop = batched(params, xs, ys), [per_ex(params, xs[i], ys[i]) for i in range(BATCH)]
    diffs = [(jnp.max(jnp.abs(v[i] - w)), jnp.sum(v[i] != w), v[i].size)
             for i, one in enumerate(loop)
             for v, w in zip(jax.tree.leaves(vec), jax.tree.leaves(one))]
    return {"t_vmap": timed(lambda: batched(params, xs, ys)),
            ...
            "worst": float(max(d[0] for d in diffs)),
            "scale": float(max(jnp.max(jnp.abs(leaf)) for leaf in jax.tree.leaves(vec))),
            "ndiff": int(sum(d[1] for d in diffs)), "ntot": int(sum(d[2] for d in diffs)),
            "mean_gap": float(max(jnp.max(jnp.abs(jnp.mean(v, axis=0) - b))
                                  for v, b in zip(jax.tree.leaves(vec),
...
def verify(result):
    ...
    return [
        practice.Check("ANSWER: the examples the model gets wrong carry the biggest gradients",
                       ...
                       f"{hot['n_wrong']} misclassified examples against {hot['right']:.2f} "
                       f"over the {BATCH - hot['n_wrong']} correct ones — {hot['wrong'] / hot['right']:.2f}x. "
                       f"The three largest sit at p(true class) "
                       + ", ".join(f"{p:.3f}" for p in hot["top_p"])
                       + "; the three smallest at "
        ...
        practice.Check("MECHANISM: the output-layer error is exactly softmax - onehot",
                       ...
                       f"the per-example gradient w.r.t. `layer3.b` matches p - y elementwise "
                       f"to {hot['bias_gap']:.2e}. Every other gradient in the tree is that "
                       f"vector back-propagated, so ||g_i|| is a monotone read-out of how "
                       f"wrong the model is about example i — correlation with the "
                       f"cross-entropy loss {hot['r_loss']:.3f}"),
        practice.Check("FINDING: at initialisation the question has almost no answer",
                       ...
                       f"untrained, the 32 norms span only {cold['spread']:.2f}x "
                       f"(correlation with ||p - y|| just {cold['r_delta']:.3f}); after "
                       f"{WARM} steps they span {hot['spread']:.2f}x at {hot['r_delta']:.3f}. "
                       f"A near-uniform softmax makes p - y nearly the same vector for every "
                       f"example, so 'which examples dominate' is a property the model "
                       f"acquires, not one the data has"),
        practice.Check("FINDING: vmap does not reproduce the loop bit for bit",
                       result["ndiff"] > 0 and result["worst"] < 1e-6,
                       f"{100 * result['ndiff'] / result['ntot']:.1f}% of the "
                       f"{result['ntot']:,} gradient entries differ between "
                       f"`vmap(grad(f))` and a Python loop over `grad(f)` — but by at most "
                       f"{result['worst']:.2e} absolute against a largest gradient of "
                       f"{result['scale']:.3f}, i.e. {result['worst'] / result['scale']:.1e} "
                       f"relative. Batching rewrites 32 matrix-vector products as one "
                       f"matrix-matrix product, and float32 addition is not associative"),
        practice.Check(f"…and vmap is {speed:.0f}x faster than that loop",
                       speed > 8.0,
                       f"best of 5: vmap+jit {1e3 * result['t_vmap']:.2f} ms, plain Python "
                       f"loop {1e3 * result['t_loop']:.1f} ms, loop over a *jitted* "
                       f"per-example grad {1e3 * result['t_jit_loop']:.2f} ms. The lesson's "
                       f"'10-100x faster than a Python loop' holds against the eager loop "
                       f"({speed:.1f}x); against a loop that is itself compiled the margin "
                       f"is only {result['t_jit_loop'] / result['t_vmap']:.1f}x, so most of "
                       f"the win is dispatch overhead, not vectorisation"),
        practice.Check("CONTROL: the per-example gradients average to the batch gradient",
                       ...
                       f"{result['mean_gap']:.2e}. `loss_fn` ends in `jnp.mean`, so grad of "
                       f"the mean is the mean of the grads — the batch gradient throws away "
                       f"exactly the spread the first check measures"),
    ]
